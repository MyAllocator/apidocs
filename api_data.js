define({ "api": [
  ...
  },
  {
    ...
    "name": "BookingCreate",
    "version": "201501.0.0",
    "description": "<p>Internal Use Only.</p> <p>** Booking callback notifications are disabled **for bookings created via BookingCreate. To test booking callback notifications, you can setup BookNow in the web interface for a test property and generate test bookings.</p>",
    "parameter": {
      "fields": {
        "Request": [
          {
  ...
  },
  {
    ...
    "name": "FAQ",
    "version": "201601.0.0",
    "description": "<ul> <li><strong>How should I deal with failed channel updates?</strong> If the error is noted as non-fatal on the Error Code page then retry the update after a few minutes. Failing updates are not as rare as you would think because channels do go down and become unresponsive.</li> </ul> <p>Fatal errors should not be automatically retried as they need user intervention. Make sure to show the error message to the customer.</p> <ul> <li> <p><strong>Are there any limits on how many updates can be send at once?</strong> There is (currently) no limit, but you're strongly advised to split large updates (especially full refreshes) into smaller chunks of 30-60 days. This has the advantage that you can easily re-run failed chunks and it will make debugging problems easier.</p> </li> <li> <p><strong>I cannot log in to myallocator with the test credentials. Why?</strong> This is to avoid the settings for the test properties from being changed. You can however just register your own test property. Should you need more than the 30-day trial period let us know and we will extend it.</p> </li> <li> <p><strong>Callback: how do you know which customers belong to us?</strong> The first time you send an update on behalf of a customer through our API this customer will be marked as a user of your PMS. From then on you will receive callbacks for this customer.</p> </li> <li> <p><strong>Callback: why do I not receive any bookings?</strong> Make sure the property is associated with your vendor account. You can check this by going to the <em>PMS Connections</em> page inside the myallocator website for the property in question to check that your PMS is listed. Also make sure to set your callback URL with the <em>VendorSet</em> call <strong>and</strong> set <em>Callback/NotifyBooking</em> to true.</p> </li> <li> <p><strong>Callback: how do I test the callback?</strong> You can use the free BookNow widget inside the myallocator website. Add availability and rates, go to MANAGE, enable BookNow and then create bookings through the preview.</p> </li> <li> <p><strong>Can I send updates in parallel?</strong> You can for different properties. What you should never do is send an update for a property without waiting for the previous update for this property to succeed/fail. Otherwise you can run into race-conditions where the later update succeeds before the first update, opening up availability on the channel unintentionally.</p> </li> <li> <p><strong>Why can there be multiple RoomTypeIds for a booking?</strong> The same channel room can be assigned to multiple myallocator rooms. For this scenario we send the sum of available rooms/beds and the highest price of any available room that is mapped to this channel room. So the list of RoomTypeIds represents all matching rooms, but it does not mean that all of them were actually booked.</p> </li> </ul> <p>Check out the warning on the Automatic Adjustments developer page for more details on how to adjust for this scenario.</p> <ul> <li> <p><strong>Do I need PCI Certification for BookingPaymentDownload?</strong> Yes. We require a PCI compliance certificate from you. Most hostel and small PMS companies (those with less than $10m year in revenue) can use a Level 3 &quot;self&quot; certification. Level 3 PCI Compliance simply requires you attest that you are following secure coding practices in your application, and server hosting environment. In most cases it will require you attain attestation from your server hosting and gateway vendors to ensure a complete web of trust. More information: http://www.pcicompliance101.net/pci-dss-compliance.htm</p> </li> <li> <p><strong>For our testing purpose, will you give us an appropriate account?</strong> You are welcome to create a trial account and use that for testing, if you would like it extended you can talk to your relationship manager. (devhelp can't give you a free account, only the relationship manager can do that)</p> </li> <li> <p><strong>In some Methods the <em>PropertyId</em> is required, where can we get this?</strong> It is returned when a property is created, or via: PropertyList</p> </li> <li> <p><strong>In the Method &quot;AssociateUserToPMS&quot; returns the error: &quot;ErrorMsg=Auth/PropertyId or Auth/PropertyToken is required&quot;; neither of these is mentioned in the specs.</strong> A single user may have multiple properties associated to them, each property has a unique number called the &quot;PropertyId&quot;</p> </li> <li> <p><strong>Some API calls found in the documentation require special vendor permissions. How may we acquiring them?</strong> Speak with your relationship manager, very few integrations need those, only very deep integrations with thousands of users. It is much better (and more common) to have your user sign up on www.myallocator.com, create property, setup channels, etc. then provide you with a username+password which you exchange for a UserToken (that doesn't expire).</p> </li> <li> <p><strong>What API calls should absolutely be implemented?</strong> The minimum we recommend to integrate is <em>RoomList</em>, <em>ARIUpdate</em> and <em>ARIUpdateStatus</em>. If you don't use booking callbacks (which you really should) you also need <em>BookingList</em>. <em>AssociateUserToPMS</em> and <em>AssociatePropertyToPMS</em> is needed for each new connection and therefore also mandatory. <em>PropertyList</em> is highly recommended to retrieve the <em>PropertyId</em> for the properties that each user has.</p> </li> </ul>",
    "filename": "perllib/MAAPI.pm",
    "groupTitle": "4_Appendix"
  },
  {
